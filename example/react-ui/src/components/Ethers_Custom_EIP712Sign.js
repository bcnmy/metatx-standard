...
function App() {
  ...
  const { walletProvider, signer, connect, web3Modal } = useWalletProvider();
    ...
    if (newQuote != "" && contract) {
      ...
      try {
        if (metaTxEnabled) {
          ...
          let transactionHash;
          try {
            let receipt = await walletProvider.sendTransaction(tx);
            console.log(receipt);
          } catch (error) {
          ...
          if (transactionHash) {
            ...
            let receipt = await walletProvider.waitForTransaction(
              transactionHash
            );
            console.log(receipt);
            showSuccessMessage("Transaction confirmed on chain");
